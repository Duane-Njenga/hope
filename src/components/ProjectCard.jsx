...
function ProjectCard({ project }) {
  ...
  useEffect(() => {
    if (user) {
      fetch(`http://localhost:5555/volunteers/check?user_id=${user.id}&event_id=${project.id}`)
        .then(res => res.json())
  ...
  const handleClick = () => {
    ...
    console.log({ user_id: user.id, event_id: project.id })
    console.log(user);
    
    const method = volunteered ? "DELETE" : "POST";
    fetch("http://localhost:5555/volunteers" + (volunteered ? `?user_id=${user.id}&event_id=${project.id}` : ""), {
      method,
